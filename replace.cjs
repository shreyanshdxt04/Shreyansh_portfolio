...
files.forEach(file => {
  ...
  // Replace colors and class names globally
  ...
  // Specific fix for index.css rgba color and variable name
  if (file.endsWith('index.css')) {
    content = content.replace(/rgba\(34, 211, 238, 0.5\)/g, 'rgba(255, 140, 0, 0.5)');
    content = content.replace(/--color-neon-cyan/g, '--color-neon-orange-alt');
  }
